...
const PropertyEditor: React.FC<PropertyEditorProps> = ({
  ...
  const { control, setValue, getValues, setError, clearErrors } = form;
  ...
  const isKeyUnique = (key: string): boolean => {
    return !getValues('properties').some(
  ...
  const onSubmit = (data: PropertyEditorForm) => {
    ...
    const updatedProperties = [
      ...
      { key: data.newKey, value: parsedValue },
    ];
  ...
  const editProperty = (index: number, key: string, value: string | number) => {
    ...
    const keyExists = currentProperties.some(
      (prop, i) => i !== index && prop.key.toLowerCase() === key.toLowerCase()
    );
      ...
      <div className='max-w-7xl mx-auto'>
        <div className='text-center mb-8'>
          ...
          <p className='text-gray-600 text-sm sm:text-base lg:text-lg max-w-2xl mx-auto'>
            Create, edit, and visualize properties for your design elements with
            real-time feedback
          </p>
        </div>
              ...
              <CardContent className='p-4 sm:p-6'>
                <section aria-labelledby='visualization-heading'>
                  ...
                  <div id='visualization-description' className='sr-only'>
                    Visual representation of your properties. Color property
                    affects background color, size property affects dimensions,
                    and label property affects the displayed text.
                  </div>
                </section>
              </CardContent>
            </Card>
